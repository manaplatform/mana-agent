...
class Settings(BaseSettings):
    ...
    openrouter_http_referer: str = Field(default="https://github.com/mana-agent/mana-agent", alias="OPENROUTER_HTTP_REFERER")
    openrouter_title: str = Field(default="Mana-Agent", alias="OPENROUTER_TITLE")
    openrouter_provider_preferences: dict[str, Any] | str = Field(default_factory=dict, alias="OPENROUTER_PROVIDER_PREFERENCES")
    ...
    mana_adaptive_routing_enabled: bool = Field(default=True, alias="MANA_ADAPTIVE_ROUTING_ENABLED")
    mana_model_profiles: list[dict[str, Any]] | str = Field(default_factory=list, alias="MANA_MODEL_PROFILES")
    mana_routing_complexity_threshold: str = Field(default="high", alias="MANA_ROUTING_COMPLEXITY_THRESHOLD")
    mana_routing_risk_threshold: str = Field(default="high", alias="MANA_ROUTING_RISK_THRESHOLD")
    mana_routing_max_candidates: int = Field(default=2, alias="MANA_ROUTING_MAX_CANDIDATES")
    mana_routing_min_confidence: float = Field(default=0.55, alias="MANA_ROUTING_MIN_CONFIDENCE")
    mana_routing_task_token_budget: int = Field(default=32_000, alias="MANA_ROUTING_TASK_TOKEN_BUDGET")
    mana_routing_task_cost_budget: float | None = Field(default=None, alias="MANA_ROUTING_TASK_COST_BUDGET")
    mana_routing_session_cost_budget: float | None = Field(default=None, alias="MANA_ROUTING_SESSION_COST_BUDGET")
    mana_routing_competition_cost_budget: float | None = Field(default=None, alias="MANA_ROUTING_COMPETITION_COST_BUDGET")
    mana_routing_verification_cost_budget: float | None = Field(default=None, alias="MANA_ROUTING_VERIFICATION_COST_BUDGET")
    mana_routing_retry_cost_budget: float | None = Field(default=None, alias="MANA_ROUTING_RETRY_COST_BUDGET")
    mana_routing_verification_reserve_ratio: float = Field(default=0.15, alias="MANA_ROUTING_VERIFICATION_RESERVE_RATIO")
    mana_routing_benchmark_weights: dict[str, float] | str = Field(default_factory=dict, alias="MANA_ROUTING_BENCHMARK_WEIGHTS")
    mana_routing_language_preferences: dict[str, Any] | str = Field(default_factory=dict, alias="MANA_ROUTING_LANGUAGE_PREFERENCES")
    mana_routing_evidence_retention_days: int = Field(default=90, alias="MANA_ROUTING_EVIDENCE_RETENTION_DAYS")
    mana_routing_circuit_breaker_failures: int = Field(default=3, alias="MANA_ROUTING_CIRCUIT_BREAKER_FAILURES")
    mana_routing_circuit_breaker_window_seconds: int = Field(default=900, alias="MANA_ROUTING_CIRCUIT_BREAKER_WINDOW_SECONDS")
    mana_routing_reliability_decay_seconds: int = Field(default=3600, alias="MANA_ROUTING_RELIABILITY_DECAY_SECONDS")
    mana_routing_model_failure_penalty_weight: float = Field(default=0.08, alias="MANA_ROUTING_MODEL_FAILURE_PENALTY_WEIGHT")
    mana_routing_provider_failure_penalty_weight: float = Field(default=0.04, alias="MANA_ROUTING_PROVIDER_FAILURE_PENALTY_WEIGHT")
    mana_gateway_routing_enforced: bool = Field(default=True, alias="MANA_GATEWAY_ROUTING_ENFORCED")
    mana_routing_simple_default: bool = Field(default=True, alias="MANA_ROUTING_SIMPLE_DEFAULT")
    mana_routing_multi_agent_enabled: bool = Field(default=False, alias="MANA_ROUTING_MULTI_AGENT_ENABLED")
    mana_routing_parallel_enabled: bool = Field(default=False, alias="MANA_ROUTING_PARALLEL_ENABLED")
    mana_routing_min_parallel_evidence: float = Field(default=0.65, alias="MANA_ROUTING_MIN_PARALLEL_EVIDENCE")
    mana_routing_max_task_tree_depth: int = Field(default=3, alias="MANA_ROUTING_MAX_TASK_TREE_DEPTH")
    mana_routing_max_concurrent_tasks: int = Field(default=4, alias="MANA_ROUTING_MAX_CONCURRENT_TASKS")
    mana_routing_task_timeout_seconds: int = Field(default=1800, alias="MANA_ROUTING_TASK_TIMEOUT_SECONDS")
    mana_routing_stall_timeout_seconds: int = Field(default=300, alias="MANA_ROUTING_STALL_TIMEOUT_SECONDS")
    mana_routing_cancellation_timeout_seconds: int = Field(default=30, alias="MANA_ROUTING_CANCELLATION_TIMEOUT_SECONDS")
    mana_routing_state_retention_days: int = Field(default=30, alias="MANA_ROUTING_STATE_RETENTION_DAYS")
    mana_routing_detail_level: str = Field(default="concise", alias="MANA_ROUTING_DETAIL_LEVEL")
    ...
    mana_computer_control_enabled: bool = Field(default=False, alias="MANA_COMPUTER_CONTROL_ENABLED")
