...
def render(_root: Path | None = None) -> None:
    ...
    st.caption(
        "Persistent registered services are independent from dashboard lifetime and agent tasks."
    )
    rows = manager.list()
    if not rows:
    ...
    for row in rows:
        with st.container(border=True):
            ...
            st.caption(
                f"Command: `{row.command_identifier}` · PID: `{row.os_pid or 'none'}` · owner: `{row.ownership}`"
            )
            ...
            if actions[0].button(
                "Stop",
                key=f"stop_{row.process_id}",
                disabled=row.state not in {"starting", "running"},
            ):
                manager.stop(row.process_id)
                st.rerun()
            if actions[1].button(
                "Restart",
                key=f"restart_{row.process_id}",
                disabled=row.state not in {"stopped", "failed", "stale", "running"},
            ):
