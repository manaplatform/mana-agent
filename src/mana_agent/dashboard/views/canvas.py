...
def render(root: Path | None = None) -> None:
    ...
    st.caption(
        "Durable A2UI surfaces and workflow activity from the shared authenticated event stream."
    )
    ...
    if not conversations:
        ...
        return
    labels = {
        f"{item.title[:48]} · {item.conversation_id[-8:]}": item.conversation_id
    ...
    if not api_base:
        st.warning(
            "Live Canvas requires the dashboard API base configured in the sidebar."
        )
        return
