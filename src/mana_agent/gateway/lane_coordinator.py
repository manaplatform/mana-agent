...
class LaneCoordinator:
    ...
    def __init__(
        ...
        *,
        ...
        self.repository = self.taskboard.store.repository
        from mana_agent.persistence.migration import TaskboardGatewayMigrator
        migrator = TaskboardGatewayMigrator(self.taskboard.store.workspace_id, db=self.taskboard.store.db)
        if not migrator.is_gateway_migrated():
            migrator.migrate_gateway()
    ...
    def _load(self) -> None:
        ...
        self._waiters = [dict(item) for item in self.repository.list_waiters()]
        self._wait_sequence = max((int(item.get("sequence", 0)) for item in self._waiters), default=0)
        for lease in self.repository.list_locks():
    ...
    def _persist_locked(self) -> None:
        for item in self._executions.values():
            self.repository.save_execution(item)
        self.repository.save_waiters(self._waiters)
    ...
    def _load_locks_file_locked(self) -> None:
        self._locks = {lease.lease_id: lease for lease in self.repository.list_locks()}

    def _persist_locks_file_locked(self) -> None:
        self.repository.clear_locks()
        for lease in self._locks.values():
            self.repository.save_lock(lease)
